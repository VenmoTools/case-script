...
class Sentences:
    @classmethod
    def make(cls, kw: str, **kwargs):
        return getattr(cls, kw)(**kwargs)


class SituationSentences(Sentences):
    @staticmethod
    def in_status(status: str) -> str:
        return ConstSentences.SITUATION_STATUS.format(status=status)

    @staticmethod
    def in_situation(situation: str) -> str:
        return ConstSentences.SITUATION.format(situation=situation)

    @staticmethod
    def when(when: str) -> str:
        return ConstSentences.SITUATION_WHEN.format(when=when)
...
class LocationSentences:
    ...
    def in_somewhere(cls, some_where):
        f = cls()
        f.msg = ConstSentences.IN_SOMEWHERE.format(some_where=some_where)
        return f

    @classmethod
    def in_particular_somewhere(cls, particular, some_where):
        f = cls()
        f.msg = ConstSentences.IN_PARTICULAR_SOMEWHERE.format(particular=particular, some_where=some_where)
        return f

    @classmethod
    def left_somewhere(cls, some_where):
        f = cls()
        f.msg = ConstSentences.LEFT_SOMEWHERE.format(some_where=some_where)
        return f

    @classmethod
    def left_particular_somewhere(cls, particular, some_where):
        f = cls()
        f.msg = ConstSentences.LEFT_SOMEWHERE.format(particular=particular, some_where=some_where)
        return f
    ...
    def right_particular_somewhere(cls, particular, some_where):
        f = cls()
        f.msg = ConstSentences.LEFT_PARTICULAR_SOMEWHERE.format(particular=particular, some_where=some_where)
        return f

    @classmethod
    def top_somewhere(cls, some_where):
        f = cls()

        f.msg = ConstSentences.ABOVE_SOMEWHERE.format(some_where=some_where)
        return f

    @classmethod
    def top_particular_somewhere(cls, particular, some_where):
        f = cls()
        f.msg = ConstSentences.ABOVE_PARTICULAR_SOMEWHERE.format(particular=particular, some_where=some_where)
        return f

    @classmethod
    def bottom_somewhere(cls, some_where):
        f = cls()
        f.msg = ConstSentences.BOTTOM_SOMEWHERE.format(some_where=some_where)
        return f

    @classmethod
    def bottom_particular_somewhere(cls, particular, some_where):
        f = cls()
        f.msg = ConstSentences.BOTTOM_PARTICULAR_SOMEWHERE.format(particular=particular, some_where=some_where)
        return f

    @classmethod
    def bottom_right(cls):
        f = cls()
        f.msg = ConstSentences.BOTTOM_RIGHT
        return f

    @classmethod
    def bottom_left(cls):
        f = cls()
        f.msg = ConstSentences.BOTTOM_LEFT
        return f

    @classmethod
    def top_right(cls):
        f = cls()
        f.msg = ConstSentences.TOP_RIGHT
        return f

    @classmethod
    def top_left(cls):
        f = cls()
        f.msg = ConstSentences.TOP_LEFT
        return f

    @classmethod
    def top(cls):
        f = cls()
        f.msg = ConstSentences.TOP
        return f

    @classmethod
    def bottom(cls):
        f = cls()
        f.msg = ConstSentences.BOTTOM
        return f

    @classmethod
    def middle(cls):
        f = cls()
        f.msg = ConstSentences.MIDDLE
        return f

    @classmethod
    def left(cls):
        f = cls()
        f.msg = ConstSentences.LEFT
        return f

    @classmethod
    def right(cls):
        f = cls()
        f.msg = ConstSentences.RIGHT
        return f


class ActionSentences(Sentences):

    @staticmethod
    def click_something(name: str) -> str:
        return ConstSentences.CLICK_NAME.format(name=name)

    @staticmethod
    def click_something_with_location(name: str, location: LocationSentences) -> str:
        return ConstSentences.CLICK_WITH_LOCATION.format(name=name, location=location.msg)

    @staticmethod
    def click_somethings_item(name: str, some_thing: str, ) -> str:
        return ConstSentences.CLICK_WITH_SOMETHING.format(name=name, some_thing=some_thing)

    @staticmethod
    def input_something(name: str, value: str, ) -> str:
        return ConstSentences.INPUT_NAME.format(name=name, value=value)

    @staticmethod
    def input_something_with_location(name: str, location: LocationSentences, value: str) -> str:
        return ConstSentences.INPUT_WITH_LOCATION_OR_SOMETHING.format(name=name, location=location.msg, value=value)

    @staticmethod
    def drag_something_to_somewhere(name: str, some_where: str) -> str:
        return ConstSentences.DRAG_SOMETHING.format(name=name, some_where=some_where)

    @staticmethod
    def drag_particular_something_to_somewhere(name: str, from_location: LocationSentences, some_where: str) -> str:
        return ConstSentences.DRAG_PARTICULAR_SOMETHING_TO_SOMEWHERE.format(name=name, from_location=from_location.msg,
    ...
    def drag_particular_something_to_particular_somewhere(name: str, from_location: LocationSentences,
                                                          ...
                                                          some_where: str) -> str:
        return ConstSentences.DRAG_PARTICULAR_SOMETHING_TO_PARTICULAR_SOMEWHERE.format(name=name,
    ...
    def drag_something_to_particular_somewhere(name: str, from_location: LocationSentences, some_where: str) -> str:
        return ConstSentences.DRAG_SOMETHING_TO_PARTICULAR_SOMEWHERE.format(name=name, from_location=from_location.msg,
                                                                            some_where=some_where)

    @staticmethod
    def select_name(name: str) -> str:
        return ConstSentences.SELECT_NAME.format(name=name)

    @staticmethod
    def select_name_with_desc(name: str, desc: str) -> str:
        return ConstSentences.SELECT_NAME_WITH_DESC.format(name=name, description=desc)

    @staticmethod
    def select_somewhere(name: str, location: LocationSentences) -> str:
        return ConstSentences.SELECT_SOMEWHERE.format(name=name, location=location.msg)

    @staticmethod
    def select_somewhere_with_desc(name: str, location: LocationSentences, desc: str) -> str:
        return ConstSentences.SELECT_SOMEWHERE_WITH_DESC.format(name=name, location=location.msg, description=desc)

    @staticmethod
    def select_particular_something(name: str, location: LocationSentences) -> str:
        return ConstSentences.SELECT_PARTICULAR_SOMETHING.format(name=name, location=location.msg)

    @staticmethod
    def open_name(name: str) -> str:
        return ConstSentences.OPEN_NAME.format(name=name)

    @staticmethod
    def open_somewhere(name: str, location: LocationSentences) -> str:
        return ConstSentences.OPEN_SOMEWHERE.format(name=name, location=location.msg)

    @staticmethod
    def open_situation(name: str, situation: str) -> str:
        return ConstSentences.OPEN_SITUATION.format(name=name, situation=situation)

    @staticmethod
    def forward(some_where: str) -> str:
        return ConstSentences.FORWARD.format(some_where=some_where)

    @staticmethod
    def back_to(some_where: str) -> str:
        return ConstSentences.BACK_TO.format(some_where=some_where)

    @staticmethod
    def jump_to(some_where: str) -> str:
        return ConstSentences.JUMP_TO.format(some_where=some_where)

    @staticmethod
    def return_to(some_where: str) -> str:
        return ConstSentences.RETURN_TO.format(some_where=some_where)

    @staticmethod
    def repeat_no_step(no: int) -> str:
        return ConstSentences.REPEAT_STEP.format(no=no)

    @staticmethod
    def find_then_do(name: str, action: str) -> str:
        return ConstSentences.REPEAT_STEP.format(name=name, action=action)
...
class ExceptSentences(Sentences):

    @staticmethod
    def except_display_something(something: str) -> str:
        return ConstSentences.EXCEPT_DISPLAY.format(something=something)

    @staticmethod
    def except_location_display_something(something: str, location: LocationSentences) -> str:
        return ConstSentences.EXCEPT_SOMEWHERE_DISPLAY.format(something=something, location=location.msg)

    @staticmethod
    def except_display_particular_somewhere(something: str, location: LocationSentences, somewhere: str) -> str:
        return ConstSentences.EXCEPT_PARTICULAR_SOMEWHERE_DISPLAY.format(something=something, somewhere=somewhere,
                                                                         location=location.msg)

    @staticmethod
    def except_result_is(something: str, name: str) -> str:
        return ConstSentences.EXCEPT_RESULT_IS.format(something=something, name=name)

    @staticmethod
    def except_status(something: str, status: StatusSentences) -> str:
        return ConstSentences.EXCEPT_STATUS.format(something=something, status=status)

    @staticmethod
    def except_show_something_at(location: LocationSentences, something: str) -> str:
        return ConstSentences.EXCEPT_SHOW_SOMETHING.format(location=location.msg, something=something)

    @staticmethod
    def except_show_something_in(location: LocationSentences, something: str) -> str:
        return ConstSentences.EXCEPT_SHOW_SOMETHING_IN_SOMEWHERE.format(location=location.msg, something=something)

    @staticmethod
    def except_change_to(something: str, to_something: str) -> str:
        return ConstSentences.EXCEPT_CHANGE.format(something=something, to_something=to_something)

    @staticmethod
    def except_note(note: str) -> str:
        return ConstSentences.EXCEPT_NOTE.format(something=note)

    @staticmethod
    def except_note_at(location: LocationSentences, note: str) -> str:
        return ConstSentences.EXCEPT_NOTE_AT_SOMEWHERE.format(location=location.msg, something=note)

    @staticmethod
    def except_include(somewhere: str, something: str) -> str:
        return ConstSentences.EXCEPT_INCLUDE.format(somewhere=somewhere, something=something)

    @staticmethod
    def except_exists(somewhere: str, something: str, description: str) -> str:
        return ConstSentences.EXCEPT_EXISTS.format(somewhere=somewhere, something=something, description=description)


class ConditionSentences(Sentences):

    @staticmethod
    def request_status(something: str, status: str) -> str:
        return ConstSentences.CONDITION_REQUEST_STATUS.format(something=something, status=status)

    @staticmethod
    def request_not_status(something: str, status: str) -> str:
        return ConstSentences.CONDITION_REQUEST_NOT_STATUS.format(something=something, status=status)

    @staticmethod
    def condition_and(other_condition: str):
        return ConstSentences.CONDITION_AND.format(condition=other_condition)

    @staticmethod
    def condition_or(other_condition: str):
        return ConstSentences.CONDITION_OR.format(condition=other_condition)

    @staticmethod
    def value_is(something: str, value: str):
        return ConstSentences.CONDITION_VALUE_IS.format(something=something, value=value)

    @staticmethod
    def content_is(something: str, content: str):
        return ConstSentences.CONDITION_CONTENT_IS.format(something=something, content=content)

    @staticmethod
    def in_list(something: str, li: List[str]):
        lists = "\n".join(map(to_str, [(i, item) for i, item in enumerate(li)]))
        return ConstSentences.CONDITION_IN_LIST.format(something=something, lists="\n" + lists)

    @staticmethod
    def exists_or_not(number: str, unit: str, something: str, description="", not_exists=False):
        word = "不" if not_exists else ""
        return ConstSentences.CONDITION_EXISTS.format(not_exists=word, number=number, unit=unit,
                                                      something=something,
    ...
    def exists_in_somewhere_or_not(location: LocationSentences, number: str, unit: str, something: str,
                                   ...
                                   not_exists=False):
        word = "不" if not_exists else ""
        return ConstSentences.CONDITION_EXISTS_IN_SOMEWHERE.format(location=location.msg, not_exists=word,
                                                                   number=number,
                                                                   unit=unit,
                                                                   something=something,
                                                                   description=description)
    ...
    def when_exists_or_not(when: str, number: str, unit: str, something: str, description: str, not_exists=False):
        word = "不" if not_exists else ""
        return ConstSentences.CONDITION_EXISTS_WITH_WHEN.format(when=when, not_exists=word, number=number, unit=unit,
                                                                something=something,
                                                                description=description)
    ...
    def when_exists_in_somewhere_or_not(when: str, somewhere: str, number: str, unit: str, something: str,
                                        ...
                                        not_exists=False):
        word = "不" if not_exists else ""
        return ConstSentences.CONDITION_EXISTS_IN_SOMEWHERE.format(when=when, somewhere=somewhere, not_exists=word,
                                                                   number=number,
                                                                   unit=unit,
                                                                   something=something,
                                                                   description=description)
...
if __name__ == '__main__':
    print(ConditionSentences.make("request_status", something="abc", status="aaa"))
